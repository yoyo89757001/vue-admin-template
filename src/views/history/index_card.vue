<template>
  <div>
    <el-row class="warpe">
          ...
          <el-col :span="3">
            <el-date-picker  type="date" placeholder="结束时间" v-model="endTime" style="width: 100%" ></el-date-picker>
          </el-col>

          <el-col :span="3" style="text-align: right;justify-items: right;justify-content: right">
            <el-input
              style="width: 8vw;margin: 1vw;align-items: center"
              fixed="right"
              placeholder="请输入姓名"
              v-model="input"
              clearable>
  ...
  export default {
    ...
    methods: {
      ...
      handleCurrentChange(val) {//点击分页时调用的方法
        ...
        getHistroy({page:this.currentPage-1,size:this.pageSize,name:this.input,type:2,startTime:Date.parse(this.startTime),endTime:Date.parse(this.endTime)}).then(response => {
          const  {data,errorCode} =response;
          const  {requestData,total} =JSON.parse(data);
          mythis.totalNum=total;
          if (requestData.length>0){
